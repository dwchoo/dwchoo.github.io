...
2026-09-07 배포 전 로컬에서 수행한 구현 및 검증 기록입니다.
...
- PLY 크기: 6,798,975바이트 (약 6.8 MB), 453,253점
- localhost 페이지 탐색 시작부터 첫 실제 render 호출까지: 데스크톱 **536.1 ms**, 모바일 **552 ms**
...
| 항목 | 결과 |
| --- | --- |
| 원본과 PLY의 전체 XYZ·RGB·순서 | 453,253행 전체 바이트 일치, 원본 SHA-256 유지 |
| 변환기 유효성 검사 | 정상 왕복·잘못된 dtype/크기/점 수/좌표/카메라/완료 상태 등 11개 테스트 통과 |
